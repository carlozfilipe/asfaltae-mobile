...
var AbstractSqliteQueryRunner = /** @class */ (function (_super) {
                ...
                switch (_a.label) {
                    case 0:
                        if (this.driver.transactionSupport === "none")
                            throw new TypeORMError("Transactions aren't supported by ".concat(this.connection.driver.options.type, "."));
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 3:
                        ...
                        up = new Query("ALTER TABLE ".concat(this.escapePath(oldTable.name), " RENAME TO ").concat(this.escapePath(newTableName)));
                        down = new Query("ALTER TABLE ".concat(this.escapePath(newTableName), " RENAME TO ").concat(this.escapePath(oldTable.name)));
    ...
    AbstractSqliteQueryRunner.prototype.createIndexSql = function (table, index) {
        ...
        return new Query("CREATE ".concat(index.isUnique ? "UNIQUE " : "", "INDEX ").concat(database ? "\"".concat(database, "\".") : "").concat(this.escapePath(index.name), " ON \"").concat(tableName, "\" (").concat(columns, ") ").concat(index.where ? "WHERE " + index.where : ""));
    };
    /**
    ...
    AbstractSqliteQueryRunner.prototype.dropIndexSql = function (indexOrName) {
        ...
        return new Query("DROP INDEX ".concat(this.escapePath(indexName)));
    };
    /**
                ...
                switch (_c.label) {
                    case 0:
                        ...
                        if (migrateData) {
                            ...
                            upQueries.push(new Query("INSERT INTO ".concat(this.escapePath(newTable.name), "(").concat(newColumnNames, ") SELECT ").concat(oldColumnNames, " FROM ").concat(this.escapePath(oldTable.name))));
                            downQueries.push(new Query("INSERT INTO ".concat(this.escapePath(oldTable.name), "(").concat(oldColumnNames, ") SELECT ").concat(newColumnNames, " FROM ").concat(this.escapePath(newTable.name))));
                        }
                        // drop old table
                        upQueries.push(this.dropTableSql(oldTable));
                        downQueries.push(this.createTableSql(oldTable, true));
                        // rename old table
                        upQueries.push(new Query("ALTER TABLE ".concat(this.escapePath(newTable.name), " RENAME TO ").concat(this.escapePath(tableNameOld))));
                        downQueries.push(new Query("ALTER TABLE ".concat(this.escapePath(oldTable.name), " RENAME TO ").concat(this.escapePath(tableNameNew))));
